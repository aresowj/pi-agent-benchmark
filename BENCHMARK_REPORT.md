...
The original matrix was 4 models × 6 cases = 24 pass@1 runs. The requested `LUNA_MAX` extension added six independent pass@1 runs, for 30 initial runs total. Each run used a fresh session and clean worktree at the same case-start SHA; hidden evaluators and run artifacts stayed outside worker worktrees.
...
5. **Does Terra Medium materially outperform cheaper models on hard work?** No correctness advantage was observed: Terra, Local, DeepSeek, and Luna all passed cases03–06. Speed/cost differences are descriptive only, not a proven quality advantage.
...
8. **Operational recommendation (unvalidated policy suggestion, not a benchmark-derived quality winner):** default worker = LOCAL_QWEN_Q3; cheap cloud escalation = DEEPSEEK; difficult-task escalation = LUNA_MAX or Terra Medium only when latency/capability policy justifies escalation; fallback/offline worker = LOCAL_QWEN_Q3. No slot earned a demonstrated quality-based escalation in this ceiling-effect sample.

## Review note

GPT-5.6 Sol at light thinking was reserved for reviewing this report and was not used as a worker benchmark slot, per the user’s instruction.
